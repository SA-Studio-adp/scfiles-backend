...
app.get("/api/tmdb", async (req, res) => {
  try {
    ...
    if (_memCache[url]) return res.json(_memCache[url]);

    const response = await fetch(url);
    if (!response.ok) return res.status(500).json({ error: "TMDB request failed" });
...
app.post("/api/series", async (req, res, next) => {
  try {
    ...
    series.unshift(newSeries);   // always add to top
...
app.post("/api/collections", async (req, res, next) => {
  try {
    ...
    // Place new collection at the end (spread existing first, new last)
    const { [id]: _, ...rest } = collections;
    const updated = { ...rest, [id]: { name: name.trim(), banner: banner || "", "bg-music": bgMusic || "", movies } };
